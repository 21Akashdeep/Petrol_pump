...
if ($qty_500 > 0) {
    $sql_bank = "INSERT INTO bank (denomination, pieces, amount, date_time) VALUES (500, $qty_500, $amount_500, '$date_time')";
    $conn->query($sql_bank);
}
...
foreach ($denominations as $value) {
    if ($value != 500) { // Skip ₹500 as it is already inserted into bank
        ...
        if ($qty > 0) {
            $sql_shift = "INSERT INTO second_shift (denomination, pieces, amount, date_time) VALUES ($value, $qty, $amount, '$date_time')";
...
if ($coins > 0) {
    $sql_coins = "INSERT INTO second_shift (denomination, pieces, amount, date_time) VALUES ('Coins', 1, $coins, '$date_time')";
    $conn->query($sql_coins);
}

// Insert grand total into 'transactions' table
$sql_transaction = "INSERT INTO transactions (total_amount, date_time) VALUES ($grand_total, '$date_time')";
$conn->query($sql_transaction);

// Close connection
$conn->close();

echo "Money successfully submitted!";
?>
