...
$sql = "SELECT * FROM liquida2 WHERE id = $id";
...
function getShiftCReading($conn, $col) {
    $sql = "SELECT $col FROM liquidc2 ORDER BY id DESC LIMIT 1";
    $result = $conn->query($sql);
    $data = $result->fetch_assoc();
    return isset($data[$col]) ? $data[$col] : "";
}
$shiftC_close_xg1_reading = getShiftCReading($conn, 'xg1_close_reading');
$shiftC_close_xg2_reading = getShiftCReading($conn, 'xg2_close_reading');
$shiftC_close_ms1_reading = getShiftCReading($conn, 'ms1_close_reading');
$shiftC_close_ms2_reading = getShiftCReading($conn, 'ms2_close_reading');
?>
<!DOCTYPE html>
<html lang="en">
<head>
