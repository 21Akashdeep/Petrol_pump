...
$sql = "SELECT * FROM shifta3 ORDER BY id DESC LIMIT 1";
...
// Fetch latest entry from the database
$sql = "SELECT * FROM shifta2 ORDER BY id DESC LIMIT 1";
$result = $conn->query($sql);

$latestData = $result->fetch_assoc(); // Fetch data as an associative array

// Fetch latest Shift C close reading
$sql_shiftC = "SELECT xp_close_reading FROM shiftc3 ORDER BY id DESC LIMIT 1";
...
$sql_shiftC = "SELECT ms_close_reading FROM shiftc3 ORDER BY id DESC LIMIT 1";
$result_shiftC = $conn->query($sql_shiftC);
$shiftC_data = $result_shiftC->fetch_assoc();
...
$sql_shiftA = "SELECT * FROM shifta3 ORDER BY id DESC LIMIT 1";
...
foreach ($fields as $field) {
    ...
    if ($field == "Rate") {
        ...
        echo '<div class="col-4">
                <input type="text" class="form-control" name="xp_' . $field_db . '" id="xpm3_' . $field_db . '" 
                value="' . $xp95_rate . '" readonly>
              </div>
              <div class="col-4">
                <input type="text" class="form-control" name="ms_' . $field_db . '" id="msm3_' . $field_db . '" 
                value="' . $ms_rate . '" readonly>
              </div>';
    } elseif ($field == "Paytm machine No.") {
        // Paytm Machine No. dropdown
        echo '<div class="col-4">
                <select class="form-select" name="xp_' . $field_db . '" id="xpm3_' . $field_db . '">';
        ...
        echo '<div class="col-4">
                <select class="form-select" name="ms_' . $field_db . '" id="msm3_' . $field_db . '">';
        foreach ($paytm_numbers as $number) {
        ...
        echo '<div class="col-4">
                <select class="form-select" name="xp_' . $field_db . '" idm3="xp_' . $field_db . '">';
        ...
        echo '<div class="col-4">
                ...
                name="xp_' . $field_db . '" id="xpm3_' . $field_db . '" 
                value="' . (isset($latestData['xp_' . $field_db]) ? $latestData['xp_' . $field_db] : "") . '" 
                ' . ($is_reading_difference || $is_net_sale ? 'readonly' : '') . '>
              </div>
              <div class="col-4">
                <input type="text" class="form-control" 
                name="ms_' . $field_db . '" id="msm3_' . $field_db . '" 
                value="' . (isset($latestData['ms_' . $field_db]) ? $latestData['ms_' . $field_db] : "") . '" 
                ' . ($is_reading_difference || $is_net_sale ? 'readonly' : '') . '>
              </div>';
    }
...
?>
    ...
    <div class="row mt-3 mb-3">
        <div class="col-3">
            <a href="moneycash/m3moneyreport.php" class="btn btn-primary w-100">Add Cash</a>
        </div>
        <div class="col-3">
            <a href="m3liquid/liquidlist.php" class="btn btn-success w-100">Add Liquid</a>
        </div>
        <div class="col-3">
            <a href="expensem3/explist.php" class="btn btn-warning w-100">Expenses</a>
        </div>
        <div class="col-3">
            <a href="collection/m3collectionreport.php" class="btn btn-info w-100">Collection</a>
        </div>
    </div>
    <script>
document.addEventListener("DOMContentLoaded", function() {
    ...
    ["xpm3", "msm3"].forEach(function(prefix) {
