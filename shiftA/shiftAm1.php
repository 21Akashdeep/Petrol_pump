...
$sql = "SELECT * FROM shift ORDER BY id DESC LIMIT 1";
$result = $conn->query($sql);
$latestData = $result->fetch_assoc();

// Fetch latest Shift C close reading
$sql_shiftC = "SELECT xp_close_reading FROM shiftc1 ORDER BY id DESC LIMIT 1";
...
$sql_shiftC = "SELECT ms_close_reading FROM shiftc1 ORDER BY id DESC LIMIT 1";
$result_shiftC = $conn->query($sql_shiftC);
$shiftC_data = $result_shiftC->fetch_assoc();
...
<div class="container-box">
    ...
    <form action="shiftAm1DB.php" method="POST">
        ...
        <div class="row mb-3">
            ...
            <div class="col-4">
                ...
                <input type="hidden" name="shifta1_product1"
                    value="<?php echo isset($latestData['shifta1_product1']) ? htmlspecialchars($latestData['shifta1_product1']) : 'XP-95'; ?>">
            </div>
            <div class="col-4">
        ...
        foreach ($fields as $field) {
            $field_db = strtolower(str_replace([" ", "."], "_", $field));
            ...
            if ($field == "Rate") {
                ...
                echo '<div class="col-4">
                        <input type="text" class="form-control" name="xp_' . $field_db . '" id="xp_' . $field_db . '" 
                        value="' . (isset($latestData['xp_' . $field_db]) ? $latestData['xp_' . $field_db] : "") . '" readonly>
                      </div>
                      <div class="col-4">
                        <input type="text" class="form-control" name="ms_' . $field_db . '" id="ms_' . $field_db . '" 
                        value="' . (isset($latestData['ms_' . $field_db]) ? $latestData['ms_' . $field_db] : "") . '" readonly>
                      </div>';
            } else {
                echo '<div class="col-4">
    ...
    <div class="row mt-3 mb-3">
        <div class="col-3">
            <a href="moneycash/m1moneyreport.php" class="btn btn-primary w-100">Add Cash</a>
        </div>
        <div class="col-3">
            <a href="m1liquid/liquidlist.php" class="btn btn-success w-100">Add Liquid</a>
        </div>
        <div class="col-3">
            <a href="expense/explist.php" class="btn btn-warning w-100">Expenses</a>
        </div>
        <div class="col-3">
            <a href="collection/m1collectionreport.php" class="btn btn-info w-100">Collection</a>
        </div>
    </div>
</div>
