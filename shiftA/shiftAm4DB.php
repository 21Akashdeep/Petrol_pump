...
// Retrieve form data
$shifta1_datetime = isset($_POST['datetime']) ? $_POST['datetime'] : null;
...
$shifta1_nozzle4 = $_POST['nozzle4'];
$paytm_0 = isset($_POST['paytm_0']) && is_numeric($_POST['paytm_0']) ? $_POST['paytm_0'] : 0;
$paytm_1 = isset($_POST['paytm_1']) && is_numeric($_POST['paytm_1']) ? $_POST['paytm_1'] : 0;
$paytm_2 = isset($_POST['paytm_2']) && is_numeric($_POST['paytm_2']) ? $_POST['paytm_2'] : 0;
$paytm_3 = isset($_POST['paytm_3']) && is_numeric($_POST['paytm_3']) ? $_POST['paytm_3'] : 0;

$card_0 = isset($_POST['card_0']) && $_POST['card_0'] !== '' ? intval($_POST['card_0']) : NULL;
$card_1 = isset($_POST['card_1']) && $_POST['card_1'] !== '' ? intval($_POST['card_1']) : NULL;
$card_2 = isset($_POST['card_2']) && $_POST['card_2'] !== '' ? intval($_POST['card_2']) : NULL;
$card_3 = isset($_POST['card_3']) && $_POST['card_3'] !== '' ? intval($_POST['card_3']) : NULL;


// Fields Array
$fields = ["start_reading", "close_reading", "reading_difference", "testing_less", "net_sale", "rate", "cash", "paytm_amount", "card_amount", "sortage", "surplus", "total_amount","paytm_amount","card_amount"];

// Initialize data array
$data = [];

foreach ($fields as $field) {
    $data["xg1_".$field] = !empty($_POST["xg1_".$field]) ? (is_array($_POST["xg1_".$field]) ? implode(",", $_POST["xg1_".$field]) : $_POST["xg1_".$field]) : '0';
    $data["xg2_".$field] = !empty($_POST["xg2_".$field]) ? (is_array($_POST["xg2_".$field]) ? implode(",", $_POST["xg2_".$field]) : $_POST["xg2_".$field]) : '0';
    $data["ms1_".$field] = !empty($_POST["ms1_".$field]) ? (is_array($_POST["ms1_".$field]) ? implode(",", $_POST["ms1_".$field]) : $_POST["ms1_".$field]) : '0';
    $data["ms2_".$field] = !empty($_POST["ms2_".$field]) ? (is_array($_POST["ms2_".$field]) ? implode(",", $_POST["ms2_".$field]) : $_POST["ms2_".$field]) : '0';
}


// Insert Query
$sql = "INSERT INTO shifta4 (datetime,product1,product2,product3,product4,employee1, employee2,employee3,employee4,nozzle1,nozzle2,nozzle3,nozzle4,card_0,card_1,card_2,card_3,
...
// Execute Query
if ($stmt->execute()) {
    echo "<script>alert('Data saved successfully!'); window.location.href='mainshiftA.php';</script>";
} else {
