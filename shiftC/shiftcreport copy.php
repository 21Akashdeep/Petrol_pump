...
$sqlcollection = "SELECT * FROM collectionc1";
$resultcollection = $conn->query($sqlcollection);
// $datacollection = $resultcollection->fetch_assoc();
// $total_collection = 0;
$today = date('Y-m-d'); 
$sqlsort = "SELECT * FROM shiftc1 WHERE DATE(shifta1_datetime) = '$today'";
...
<body>
    ...
    $totalc = $bank200result + $bank100result + $bank50result + $bank20result + $bank10result + $coinsresult;
...
?>
 ...
 <!-- pdf generation -->
    ...
    <table id="expenseTable">
        ...
        <tr>
            <?php $_SESSION['closingc'] = $closingc;?>
            <td><strong>Closing Balance in A-shift</strong></td>
            <td></td>
            <td><?php $closingc= $totalc+$bank500result;echo $closingc;
            ?>
    ...
    $total_collection = [];
    $grand_total = 0; // ✅ Overall Total Variable
    ...
    if ($resultcollection->num_rows > 0) {
        while ($datacollection = $resultcollection->fetch_assoc()) {
            ...
            foreach ($employees as $name => $amount) {
                ...
                $grand_total += $amount; // ✅ Overall Total Calculation
                ?>
                <tr>
